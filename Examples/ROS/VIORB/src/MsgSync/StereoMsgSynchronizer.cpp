...
StereoMsgSynchronizer::StereoMsgSynchronizer(const double &imagedelay1,
                                             const double &imagedelay2)
        : _img1Delay(imagedelay1), _img2Delay(imagedelay2) {
    ...
    ROS_INFO("Message Filter Created in offline mode");
...
StereoMsgSynchronizer::StereoMsgSynchronizer(const std::string &image1_topic,
                                             const std::string &image2_topic,
                                             const std::string &imu_topic,
                                             const double &imagedelay1) {

    _realTimeFlag = true;
    _imageMsgDelaySec = imagedelay1;
    _img1Delay = imagedelay1;

    ros::NodeHandle nh;
    _img1MsgSubFilter = std::make_shared<message_filters::Subscriber<sensor_msgs::Image> >(nh, image1_topic, 1);
    _img2MsgSubFilter = std::make_shared<message_filters::Subscriber<sensor_msgs::Image> >(nh, image2_topic, 1);
    ...
    _imu_sub = nh.subscribe(imu_topic, 100, &StereoMsgSynchronizer::addImuMsg, this);
    ROS_INFO("Message Filter Created in subscribe mode");
}
...
void StereoMsgSynchronizer::stereoSyncCallback(const sensor_msgs::ImageConstPtr &img1_msg_const,
    ...
    if (_imageMsgDelaySec >= 0) {
        ...
        if (_status == NOTINIT) {
            ...
            _status = INIT;
        } else {   // no image data if there's no imu message
            _imageMsgQueue.push(
