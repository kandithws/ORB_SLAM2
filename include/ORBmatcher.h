...
public:
    ...
    int SearchBySim3(std::shared_ptr<KeyFrame> pKF1, std::shared_ptr<KeyFrame> pKF2, std::vector<std::shared_ptr<MapPoint> > &vpMatches12, const float &s12, const cv::Mat &R12, const cv::Mat &t12, const float th);
    ...
    int Fuse(std::shared_ptr<KeyFrame> pKF, cv::Mat Scw, const std::vector<std::shared_ptr<MapPoint> > &vpPoints, float th, vector<std::shared_ptr<MapPoint> > &vpReplacePoint);
