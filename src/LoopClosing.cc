...
LoopClosing::LoopClosing(std::shared_ptr<Map> pMap, std::shared_ptr<KeyFrameDatabase> pDB, std::shared_ptr<ORBVocabulary> pVoc, const bool bFixScale):
...
bool LoopClosing::ComputeSim3()
{
    ...
    for(int i=0; i<nInitialCandidates; i++)
    {
        ...
        {
            std::shared_ptr<Sim3Solver> pSolver(new Sim3Solver(mpCurrentKF,pKF,vvpMapPointMatches[i],mbFixScale));
...
void LoopClosing::SearchAndFuse(const KeyFrameAndPose &CorrectedPosesMap)
{
    ...
    for(KeyFrameAndPose::const_iterator mit=CorrectedPosesMap.begin(), mend=CorrectedPosesMap.end(); mit!=mend;mit++)
    {
        ...
        vector<std::shared_ptr<MapPoint> > vpReplacePoints(mvpLoopMapPoints.size(),static_cast<std::shared_ptr<MapPoint> >(NULL));
