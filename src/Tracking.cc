...
Tracking::Tracking(std::shared_ptr<System> pSys, std::shared_ptr<ORBVocabulary> pVoc, std::shared_ptr<FrameDrawer> pFrameDrawer, std::shared_ptr<MapDrawer> pMapDrawer, std::shared_ptr<Map> pMap, std::shared_ptr<KeyFrameDatabase> pKFDB, const string &strSettingPath, const int sensor):
    mState(NO_IMAGES_YET), mSensor(sensor), mbOnlyTracking(false), mbVO(false), mpORBVocabulary(pVoc),
    mpKeyFrameDB(pKFDB), mpInitializer(static_cast<std::shared_ptr<Initializer> >(NULL)), mpSystem(pSys), mpViewer(NULL),
    mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0)
{
    ...
    mpORBextractorLeft = STD_MAKE_SHARED(ORBextractor, nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST);

    if(sensor==System::STEREO)
        mpORBextractorRight = STD_MAKE_SHARED(ORBextractor, nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST);

    if(sensor==System::MONOCULAR)
        mpIniORBextractor = STD_MAKE_SHARED(ORBextractor, 2*nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST);
...
Tracking::Tracking(std::shared_ptr<System> pSys,
                   ...
                   std::shared_ptr<KeyFrameDatabase> pKFDB,
            ...
            for(int i=0; i<mCurrentFrame->N; i++)
            {
                ...
                if(pMP)
                    if(pMP->Observations()<1)
                    {
                        ...
                        mCurrentFrame->mvpMapPoints[i]=static_cast<std::shared_ptr<MapPoint> >(NULL);
                    }
            }
    ...
    if(mCurrentFrame->N>500)
    {
        ...
        for(int i=0; i<mCurrentFrame->N;i++)
        {
            ...
            if(z>0)
            {
                ...
                std::shared_ptr<MapPoint> pNewMP(new MapPoint(x3D,pKFini,mpMap));
...
//            mLastFrame = Frame(mCurrentFrame);
    ...
    else
    {
        ...
        {
            mpInitializer.reset();
            mpInitializer = static_cast<std::shared_ptr<Initializer> >(NULL);
            fill(mvIniMatches.begin(),mvIniMatches.end(),-1);
            return;
        }
...
void Tracking::UpdateLastFrame()
{
    ...
    for(size_t j=0; j<vDepthIdx.size();j++)
    {
        ...
        if(bCreateNew)
        {
            ...
            std::shared_ptr<MapPoint> pNewMP( new MapPoint(x3D,mpMap,mLastFrame,i));
    ...
    if(mSensor!=System::MONOCULAR)
    {
        ...
        if(!vDepthIdx.empty())
        {
            ...
            for(size_t j=0; j<vDepthIdx.size();j++)
            {
                ...
                {
                    cv::Mat x3D = mCurrentFrame->UnprojectStereo(i);
                    std::shared_ptr<MapPoint> pNewMP(new MapPoint(x3D,pKF,mpMap));
...
void Tracking::UpdateLocalKeyFrames()
{
    ...
    int max=0;
    std::shared_ptr<KeyFrame> pKFmax= static_cast<std::shared_ptr<KeyFrame> >(NULL);
            ...
            if(!Tcw.empty())
            {
                ...
                for(int io =0; io<mCurrentFrame->N; io++)
                    if(mCurrentFrame->mvbOutlier[io])
                        mCurrentFrame->mvpMapPoints[io]=static_cast<std::shared_ptr<MapPoint> >(NULL);
