...
bool LocalMapping::GetUseIMUFlag() {
    ...
    if (!mbMonocular){
        ...
        ret &= (!mbVINSInited) && (!mbUseIMUFastInit);
    }
...
bool LocalMapping::GetUseIMUFastInit() {

    return mbUseIMU && mbUseIMUFastInit;
}
...
bool LocalMapping::TryInitVIO(void) {
    ...
    if (w.type() != I3.type() || u.type() != I3.type() || vt.type() != I3.type())
        cerr << "different mat type, I3,w,u,vt: " << I3.type() << "," << w.type() << "," << u.type() << "," << vt.type()
             << endl;
        ...
        SetUpdatingInitPoses(true);
        {
            ...
            for (vector<KeyFrame *>::const_iterator vit = vScaleGravityKF.begin(), vend = vScaleGravityKF.end();
                ...
                if (pKF != vScaleGravityKF.back()) {
                    ...
                    cv::Mat vel = -1. / dt *
                                  (scale * (wPc - wPcnext) + (Rwc - Rwcnext) * pcb + Rwc * Rcb * (dp + Jpba * dbiasa_) +
                                   0.5 * gw * dt * dt);
...
bool LocalMapping::TryInitVIOFast() {
    // bool bVIOInited = false;
    ...
    if (vIMUSize >= num_msgs+1){
        ...
        {
            // TOO lazy to write fn
            std::unique_lock<std::mutex> lock(mMutexGravityVec);
            mGravityVec = gw.rowRange(0,3).clone();
        }
...
LocalMapping::LocalMapping(Map *pMap, const bool bMonocular) :
    ...
    mbUseIMUFastInit = Config::getInstance().IMUParams().fast_init;
}
                ...
                if (mpMap->KeyFramesInMap() > 2) {
                    ...
                    if (mbUseIMU) {
                        ...
                        if (!GetVINSInited()) {
                            ...
                            if (mbUseIMUFastInit){
                                // Use this on both real-time and non-realtime
                                assert(!mbMonocular);

                                SPDLOG_INFO("Initializing FAST IMU");
                                TryInitVIOFast();
