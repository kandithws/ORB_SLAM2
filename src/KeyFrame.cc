...
{
    int idx = pMP->GetIndexInKeyFrame(_self);
    if(idx>=0)
        mvpMapPoints[idx]=static_cast<std::shared_ptr<MapPoint> >(NULL);
}
...
void KeyFrame::UpdateConnections()
{
    ...
    for(map<std::shared_ptr<KeyFrame> ,int>::iterator mit=KFcounter.begin(), mend=KFcounter.end(); mit!=mend; mit++)
    {
        ...
        {
            vPairs.push_back(make_pair(mit->second,mit->first));
            (mit->first)->AddConnection(_self,mit->second);
        }
    }
    ...
    if(vPairs.empty())
    {
        ...
        pKFmax->AddConnection(_self,nmax);
    }
...
{   
    ...
    for(map<std::shared_ptr<KeyFrame> ,int>::iterator mit = mConnectedKeyFrameWeights.begin(), mend=mConnectedKeyFrameWeights.end(); mit!=mend; mit++)
        mit->first->EraseConnection(_self);

    for(size_t i=0; i<mvpMapPoints.size(); i++)
        if(mvpMapPoints[i])
            mvpMapPoints[i]->EraseObservation(_self);
    {
        unique_lock<mutex> lock(mMutexConnections);
        ...
        mpParent->EraseChild(_self);
        mTcp = Tcw*mpParent->GetPoseInverse();
        mbBad = true;
    }


    mpMap->EraseKeyFrame(_self);
    mpKeyFrameDB->erase(_self);
}
